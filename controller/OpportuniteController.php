...
class OpportuniteController extends Controller
{
    ...
    function liste(){
        ...
        $lesMOva = array();
        $lesMOve = array();
        $lesOpportunites = $this->opportuniteInnerUtilisateurModel->find(array('conditions' => 1));
        $d['opportunites'] = $lesOpportunites;
        foreach ($lesOpportunites as $oppo) {
            $idMova = $oppo->maitriseOuvrage;
            $idMOve = $oppo->maitreOuvre;
            $leMova=$this->maitreOuvrageModel->findFirst(array('conditions' => ["id",$idMova]));
            $leMove=$this->maitriseOeuvreModel->findFirst(array('conditions' => ["id",$idMove]));
            array_push($lesMOva, $leMova->nom);
            array_push($lesMOve, $leMove->nom);
        }
        $lesMOva[0]="détail MOA"; $lesMOve[0]="détail MOE";
        $d['lesMoa'] = $lesMOva;
        $d['lesMoe'] = $lesMOve;
        $this->set($d);
    }
   ...
   /* function creerOpportunite(){
        if(isset($_POST['information']) && isset($_POST['adresse']) && isset($_POST['ville']) && isset($_POST['cp']) && isset($_POST['status'])){
           ...
           // echo isset($_POST['dateLimite']);
            ...
            $test = array("dateAjout", "information", "adresse", "ville", "cp", "idUtilisateur", "commentaire", "groupe", "status");
    ...
    function creerOpportunite2($idOp){
        //echo "nonononononnonononononnonon";
        if (isset($_POST['nomMOuvrage']) && isset($_POST['prenomMOuvrage'])) {
            echo "test teste etteetet";
            $nom = $_POST['nomMOuvrage'];
            $prenom = $_POST['prenomMOuvrage'];

            $mail = isset($_POST['emailMOuvrage']) ? $_POST['emailMOuvrage'] : "";
            $tel = isset($_POST['telMOuvrage']) ? $_POST['telMOuvrage'] : "";

            $this->maitreOuvrageModel->insertAI(
                array("nom", "prenom", "mail", "tel"),
                array($nom, $prenom, $mail, $tel));
           ...
           header("Location: " . BASE_URL . "/opportunite/creerOpportunite3/". $idOp);
            ...
            $this->opportuniteModel->update(array('conditions' => array('idOpportunite' => $idOp), 'donnees' => array("maitreOuvrage" => $id)));
            header("Location: " . BASE_URL . "/opportunite/creerOpportunite3/". $idOp);
        }
        $d["id"] = $idOp;
        $d["maitreOuvrages"] = $this->maitreOuvrageModel->find(array('conditions' => 1));
        $this->set($d);
    ...
    function creerOpportunite3($idOp){
        ...
        if (isset($_POST['nomMOeuvre']) && isset($_POST['prenomMOeuvre'])) {
           // echo"test1";
            ...
            $this->maitriseOeuvreModel->insertAI(array("nom", "prenom", "mail", "telephone"), array($nom, $prenom, $mail, $tel));

            $m = $this->maitriseOeuvreModel->findFirst(array('conditions' => "1", 'orderby' => "id DESC"));

            $this->opportuniteModel->update(array('conditions' => array('idOpportunite' => $idOp), 'donnees' => array("maitriseOeuvre" => $m->id)));
            ...
            $this->opportuniteModel->update(array('conditions' => array('idOpportunite' => $idOp), 'donnees' => array("maitriseOeuvre" => $id)));
            header("Location: " . BASE_URL . "/opportunite/liste");
        }
