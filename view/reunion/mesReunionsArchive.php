            ...
            if(isset($reunions)){
                foreach ($reunions as $p ): ?>
                    <tr>
                        ...
                        <td>
                            <div class = "inscription-button">
                                <?php if($p->organisateur == Session::get("id")): ?>
                                    <?php if(isset($p->contrendu) && strlen($p->contrendu) > 0): ?>
                                        <center><button class="fill" type="submit" formaction="<?=BASE_URL?>/reunion/contrenduReunion/<?= $p->id?>">Modifier le contrendu</button></center>
                                    <?php else: ?>
                                        <center><button class="fill" type="submit" formaction="<?=BASE_URL?>/reunion/contrenduReunion/<?= $p->id?>">Faire le contrendu</button></center>
                                    <?php endif ?>
                                <?php else: ?>
                                    <?php if(isset($p->contrendu) && strlen($p->contrendu) > 0): ?>
                                        <center><button class="fill" type="submit" formaction="<?=BASE_URL?>/reunion/contrendu/<?= $p->id?>">Afficher le contrendu</button></center>
                                    <?php else: ?>
                                        <center><button disabled class="fill" type="submit" formaction="<?=BASE_URL?>/reunion/contrendu/<?= $p->id?>">Afficher le contrendu</button></center>
                                    <?php endif ?>
                                <?php endif ?>
                            </div>
                        </td>
